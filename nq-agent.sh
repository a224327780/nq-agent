...
API="https://nodequery-atlinbing.cloud.okteto.net/save"
if [ -n "$(command -v timeout)" ]
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/nodequery/nq-agent.log -T 25 --post-data "$data_post" --no-check-certificate "$API"
else
	wget -q -o /dev/null -O /etc/nodequery/nq-agent.log -T 25 --post-data "$data_post" --no-check-certificate "$API"
	wget_pid=$! 
	wget_counter=0
	wget_timeout=30
